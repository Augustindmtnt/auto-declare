...
import { DEFAULT_CHILDREN, computeMonthlySalary } from "../constants";

describe("computeDeclaration", () => {
  const axelle = DEFAULT_CHILDREN[0]; // Axelle
  const brune = DEFAULT_CHILDREN[1]; // Brune
  const axelleSalary = computeMonthlySalary(axelle.netHourlyRate); // 658.13
  const bruneSalary = computeMonthlySalary(brune.netHourlyRate); // 691.88

  it("computes Axelle's declaration for a full February 2026", () => {
    const month = new Date(2026, 1, 1);
    const daysOff = new Set<string>();
    const result = computeDeclaration(axelle, axelleSalary, month, daysOff);

    expect(result.childName).toBe("Axelle");
    expect(result.monthlySalary).toBeCloseTo(658.13, 1);
    ...
    expect(result.adjustedSalary).toBeCloseTo(658.13, 1);
    // 658.13 + 12.87 = 671.00
    expect(result.totalSalary).toBeCloseTo(671.00, 0);
    // 20 × 4 = 80
  ...
  it("computes Brune's declaration for a full February 2026", () => {
    const month = new Date(2026, 1, 1);
    const daysOff = new Set<string>();
    const result = computeDeclaration(brune, bruneSalary, month, daysOff);

    expect(result.childName).toBe("Brune");
    expect(result.monthlySalary).toBeCloseTo(691.88, 1);
    // 3h × 4.51 = 13.53
    expect(result.majoredHoursAmount).toBeCloseTo(13.53, 2);
    expect(result.totalSalary).toBeCloseTo(705.41, 0);
  });
  ...
  it("handles a month with zero worked days", () => {
    ...
    expect(result.totalSalary).toBeCloseTo(658.13, 1);
  ...
  describe("sick leave deduction", () => {
    it("computes deduction for 2 sick days in February 2026", () => {
      ...
      const result = computeDeclaration(axelle, axelleSalary, month, daysOff, sickLeaveDays);
